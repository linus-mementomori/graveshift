# CLOUD_PLAN.md — Nightfall accounts, sync & admin
...
**The outbox.** Pending mutations queue in `localStorage['nightfall:outbox']` and flush on app
...
**3 · Create `.env.local`** in the nightfall project root:
...
Confirm `.env.local` is gitignored. (Nightfall has no `.gitignore` yet and isn't a git repo — worth
